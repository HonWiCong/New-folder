...
describe('TCityCode e2e test', () => {
	...
	describe('TCityCode page', () => {
		...
		describe('with existing value', () => {
				...
				cy.authenticatedRequest({
					...
					cy.intercept(
						...
						},
						{
							statusCode: 200,
							headers: {
								link: '<http://localhost/api/t-city-codes?page=0&size=20>; rel="last",<http://localhost/api/t-city-codes?page=0&size=20>; rel="first"',
							},
							body: [tCityCode],
						}
			...
			it.skip('edit button click should load edit TCityCode page and save', () => {
				cy.get(entityEditButtonSelector).first().click();
				cy.getEntityCreateUpdateHeading('TCityCode');
	...
	describe('new TCityCode page', () => {
		...
		it('should create an instance of TCityCode', () => {
			...
			cy.get(`[data-cy="enteredDate"]`).type('2024-01-08T22:39').blur().should('have.value', '2024-01-08T22:39');

			cy.get(`[data-cy="modifiedBy"]`).type('27594').should('have.value', '27594');

			cy.get(`[data-cy="modifiedDate"]`).type('2024-01-08T22:41').blur().should('have.value', '2024-01-08T22:41');
