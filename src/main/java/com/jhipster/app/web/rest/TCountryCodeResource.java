...
import com.jhipster.app.service.TCountryCodeQueryService;
import com.jhipster.app.service.TCountryCodeService;
...
public class TCountryCodeResource {
	...
	private final TCountryCodeService tCountryCodeService;

	private final TCountryCodeRepository tCountryCodeRepository;

	private final TCountryCodeQueryService tCountryCodeQueryService;

	public TCountryCodeResource(
		TCountryCodeService tCountryCodeService,
		TCountryCodeRepository tCountryCodeRepository,
		TCountryCodeQueryService tCountryCodeQueryService
	) {
		this.tCountryCodeService = tCountryCodeService;
		this.tCountryCodeRepository = tCountryCodeRepository;
		this.tCountryCodeQueryService = tCountryCodeQueryService;
	}
	...
	public ResponseEntity<TCountryCode> createTCountryCode(@RequestBody TCountryCode tCountryCode) throws URISyntaxException {
		...
		TCountryCode result = tCountryCodeService.save(tCountryCode);
	...
	public ResponseEntity<TCountryCode> updateTCountryCode(
		...
		TCountryCode result = tCountryCodeService.update(tCountryCode);
		return ResponseEntity
			.ok()
	...
	public ResponseEntity<TCountryCode> partialUpdateTCountryCode(
		...
		Optional<TCountryCode> result = tCountryCodeService.partialUpdate(tCountryCode);
	...
	public ResponseEntity<List<TCountryCode>> getAllTCountryCodes(
		TCountryCodeCriteria criteria,
		@org.springdoc.api.annotations.ParameterObject Pageable pageable
	) {
		log.debug("REST request to get TCountryCodes by criteria: {}", criteria);
		Page<TCountryCode> page = tCountryCodeQueryService.findByCriteria(criteria, pageable);
		HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
		return ResponseEntity.ok().headers(headers).body(page.getContent());
	}

	/**
	 * {@code GET  /t-country-codes/count} : count all the tCountryCodes.
	 *
	 * @param criteria the criteria which the requested entities should match.
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
	 */
	@GetMapping("/t-country-codes/count")
	public ResponseEntity<Long> countTCountryCodes(TCountryCodeCriteria criteria) {
		log.debug("REST request to count TCountryCodes by criteria: {}", criteria);
		return ResponseEntity.ok().body(tCountryCodeQueryService.countByCriteria(criteria));
	}
	...
	public ResponseEntity<TCountryCode> getTCountryCode(@PathVariable Long id) {
		...
		Optional<TCountryCode> tCountryCode = tCountryCodeService.findOne(id);
